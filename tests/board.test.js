...
(async () => {
  try {
    ...
    check(document.querySelectorAll('.theme-card.locked').length === 6 && !document.querySelector('.theme-card[data-theme="piraten"]').classList.contains('locked') && /Crimson Pass/.test(document.querySelector('.theme-card[data-theme="hotel"] .theme-lock').textContent), 'na 9 zaken: landhuis en piratenschip open, de zes betaalde werelden dicht met "Crimson Pass"');
    document.querySelector('.theme-card[data-theme="hotel"]').click();
    check(document.getElementById('store-modal').classList.contains('active') && /Grand Hotel Aurora/.test(document.getElementById('store-lead').textContent) && document.querySelectorAll('#store-rows .store-row').length === 8, 'tik op een betaalde wereld opent de winkel met die wereld in de kop en acht rijen');
    App.hideModal('store-modal');
    App.storageSet('crimson-store-mock', '1');
    await App.buy(window.Store.IDS.pass); App.renderThemePicker();
    check(window.Store.hasPass() && document.querySelectorAll('.theme-card.locked').length === 0 && window.Progress.freezes() === 2, 'Crimson Pass (nep-winkel): alles open en twee vrije dagen erbij');
    App.storageSet('crimson-board-stats', JSON.stringify({ solved: 24 })); window.Store.save({ pass: false, worlds: [], cosmetics: false }); window.Progress.setFreezes(0); App.renderThemePicker();
    check(document.querySelectorAll('.theme-card.locked').length === 6, 'zonder Pass: de betaalde werelden weer dicht, ook met 24 zaken');
    App.storageSet('crimson-board-stats', JSON.stringify({ solved: 9 })); App.renderThemePicker();
    document.querySelector('.theme-card[data-theme="piraten"]').click();
    ...
    check(document.getElementById('hint-badge').textContent === '2' && !document.getElementById('hint-badge').hidden, 'hintteller: na één hint nog twee gratis vandaag');
    ...
    check(document.querySelectorAll('.world-tab').length === 8 && document.querySelectorAll('.world-tab small').length === 6 && /Pass/.test(document.querySelector('.world-tab[data-theme="trein"] small').textContent) && document.querySelectorAll('.map-buy').length === 6, 'acht werelden in de kiezer; zes betaalde werelden dicht met een Pass-knop op de banner');
